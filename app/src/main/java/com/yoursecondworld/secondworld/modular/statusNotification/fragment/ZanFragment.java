...
import com.yoursecondworld.secondworld.modular.statusNotification.fragment.adapter.ZanFragmentAdapter;
 ...
 * 显示赞的列表的fragment
 */
public class ZanFragment extends BaseFragment implements IStatusNotificationView {

    /**
     * 显示咱的内容的列表控件
     */
    @Injection(R.id.rv_frag_zan_for_status_notifi_content)
    private RecyclerView rv = null;

    /**
     * 显示赞的内容的适配器
     */
    private ZanFragmentAdapter adapter = null;

    /**
     * 显示的数据
     */
    private List<ZanListEntity> data = new ArrayList<ZanListEntity>();

    private StatusNotificationPresenter statusNotificationPresenter = new StatusNotificationPresenter(this);

    @Override
    public int getLayoutId() {
        return R.layout.frag_zan_for_status_notifi;
    }
    ...
    protected void initView() {
        ...
        //创建适配器
        adapter = new ZanFragmentAdapter(context, data);
    ...
    protected void initData() {
        super.initData();
        statusNotificationPresenter.getZanList();
    }
    ...
    protected void setOnListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) { //如果暂停了
                    ...
                    if (b) {
                        ...
                        //加载更多的赞的列表
                        statusNotificationPresenter.getMoreZanList();
                    }
                }
            }
        });
        ...
        adapter.setOnViewInItemClickListener(new CommonRecyclerViewAdapter.OnViewInItemClickListener() {
            ...
            public void onViewInItemClick(View v, int position) {
                ...
                switch (id) {
                    case R.id.tv_reply:
                        ZanListEntity entity = data.get(position);
    ...
    public void onLoadZanListSuccess(List<ZanListEntity> zanListEntities) {
        isNoTAnyMore = false;
        AdapterNotify.notifyFreshData(data, zanListEntities, adapter);
    }

    @Override
    public void onLoadMoreZanListSuccess(List<ZanListEntity> zanListEntities) {
        if (zanListEntities.size() == 0) {
            isNoTAnyMore = true;
        }
        AdapterNotify.notifyAppendData(data, zanListEntities, adapter);
    }

...
//        return createScaleBitmap(src, reqWidth, reqHeight); // 进一步得到目标大小的缩略图
     ...
     * @return
     */
    public static Bitmap decodeLocalImage(Context context, int rsd, int reqWidth, int reqHeight) {
        //获取大图的参数,包括大小
        BitmapFactory.Options options = getBitMapOptions(context, rsd);
        //获取一个合适的缩放比例
        options.inSampleSize = calculateInSampleSize(options, reqWidth,
                reqHeight); // 计算inSampleSize
        //让BitmapFactory加载真的图片,等于true的时候只加载了图片的大小
        options.inJustDecodeBounds = false;
        Bitmap src = BitmapFactory.decodeResource(context.getResources(), rsd, options); // 载入一个稍大的缩略图
        return createScaleBitmap(src, reqWidth, reqHeight); // 进一步得到目标大小的缩略图
    }
...
//    }
     ...
     * @return
     */
    public static BitmapFactory.Options getBitMapOptions(Context context, int rsd) {
        final BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        //计算出了图片的大小,没有真正的加载
        BitmapFactory.decodeResource(context.getResources(), rsd, options);
        return options;
    }
